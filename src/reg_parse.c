...
#define expect_char(p, c)                                                      \
  do {                                                                         \
    int pos = cur_pos(p);                                                      \
    if (next_char(p) != (c))                                                   \
      reg_error(p->env, "expect char '%c' at pos: %d\n", (c), pos);            \
  } while (0)
#define unexpect_char(p)                                                       \
  do {                                                                         \
    char s[4] = {0};                                                           \
    s[0] = at_char(p);                                                         \
    if (s[0] == 0)                                                             \
...
void parse_clear(struct reg_parse *p) { p->nodes_cap = 0; }

void parse_free(struct reg_parse *p) {
  if (p->stream)
    stream_free(p->stream);
  free(p->nodes);
  free(p);
}

int parse_is_match_tail(struct reg_parse *p) { return p->is_match_tail; }

struct reg_ast_node *parse_exec(struct reg_parse *p, const char *rule,
                                size_t size) {
...
static inline struct reg_ast_node *_gen_node(struct reg_parse *p,
                                             enum reg_op op, ...) {
...
// for test
static char *op_map[] = {"op_nil",  "op_and", "op_or",

                         "op_rps",  "op_rpp", "op_rpq",
...
static void _parse_dump(struct reg_ast_node *root) {
  ...
  if (root->op == op_range)
    printf("[addr: %p] type: %s  value:[%c - %c] childs[%p, %p] \n", root,
           op_map[root->op], root->value.range.begin, root->value.range.end,
