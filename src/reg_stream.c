...
void stream_free(struct reg_stream *p) { free(p); }
...
inline int stream_end(struct reg_stream *p) { return p->pos >= p->size; }
...
inline size_t stream_pos(struct reg_stream *p) { return p->pos; }
