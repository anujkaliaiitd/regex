...
void state_clear(struct reg_state *p) { list_clear(p->frame_list); }
...
static inline int _campar(const struct _range_frame *a,
                          const struct _range_frame *b) {
  return a->value[0] - b->value[0];
}

struct reg_pattern *state_new_pattern(struct reg_state *p,
                                      struct reg_ast_node *ast,
                                      int is_match_tail) {
...
static void _gen_frame(struct reg_state *p, struct reg_ast_node *root) {
  ...
  if (root->op == op_range) {
    ...
    struct _range_frame frame1 = {
        .type = e_begin, .value = {begin, end},
    };

    struct _range_frame frame2 = {
        .type = e_end, .value = {end, begin},
    };
...
static inline void _insert_edge(struct reg_pattern *pattern, int begin,
                                int end) {
...
static inline int _read_frame(struct reg_pattern *pattern, size_t idx,
  ...
  *out_next_begin = value;
  ...
  struct _range_frame *v = NULL;
  for (; v = frame_idx(pattern->state, idx), v && value == v->value[0];
       idx++, count++) {
...
static inline void _dump_node(struct reg_pattern *pattern,
                              struct reg_node *node) {
  printf("state[%zd] merge:%zd end: %d  edges: %zd", node->node_pos,
         node->merge_pos, node->is_end, list_len(node->edges));
  ...
  for (size_t i = 0; (path = list_idx(node->edges, i)); i++) {
    if (path->edge_pos > 0) {
      ...
      printf("[%c-%c]->next_state<%zd>  ", edge->range.begin, edge->range.end,
             path->next_node_pos);
    } else {
