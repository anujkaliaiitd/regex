...
#define foreach_edge(v, node)                                                  \
  struct _reg_path *v = NULL;                                                  \
...
static inline size_t _node_pass(struct reg_pattern *pattern, size_t node_pos,
                                size_t edge_pos);
static inline void _node_tag(struct reg_pattern *pattern, size_t node_pos,
                             int tag);
static inline int _node_insert(struct reg_pattern *pattern, size_t node_pos,
                               struct reg_range *range, size_t dest_pos);
static inline size_t _node_new(struct reg_pattern *pattern);
static int __move(struct reg_pattern *pattern, struct reg_list *subset,
                  size_t edge_pos, struct reg_list *out_subset);

/*dump op*/
static size_t _gen_op(struct reg_pattern *pattern, struct reg_ast_node *root,
                      size_t start_state_pos);
static size_t _gen_op_and(struct reg_pattern *pattern,
                          struct reg_ast_node *root, size_t start_state_pos);
static size_t _gen_op_or(struct reg_pattern *pattern, struct reg_ast_node *root,
                         size_t start_state_pos);

static size_t _gen_op_rps(struct reg_pattern *pattern,
                          struct reg_ast_node *root, size_t start_state_pos);
static size_t _gen_op_rpp(struct reg_pattern *pattern,
                          struct reg_ast_node *root, size_t start_state_pos);
static size_t _gen_op_rpq(struct reg_pattern *pattern,
                          struct reg_ast_node *root, size_t start_state_pos);

static size_t _gen_op_range(struct reg_pattern *pattern,
                            struct reg_ast_node *root, size_t start_state_pos);
...
//------------------------------ check char at edge
//------------------------------
inline struct reg_edge *state_edge_pos(struct reg_pattern *pattern,
                                       size_t pos) {
  if (pos == 0)
    return NULL; // is edsilone edege
...
inline struct reg_node *state_node_pos(struct reg_pattern *pattern,
                                       size_t pos) {
  if (!pos)
    return NULL;
...
static inline void __node_insert(struct reg_pattern *pattern, size_t node_pos,
...
static inline int _node_insert(struct reg_pattern *pattern, size_t node_pos,
  ...
  for (size_t i = 0; (edge = list_idx(pattern->edges_list, i)); i++) {
    if (count > 0 && range->begin > edge->range.end)
      break;
...
static int _campar(size_t *a, size_t *b) { return *a - *b; }
...
*/
static int _move(struct reg_pattern *pattern, size_t state_pos,
                 size_t edge_pos) {
...
*/
static void _dfa_node_insert(struct reg_pattern *pattern, size_t node_pos,
                             size_t edge_pos, size_t next_node_pos) {
  struct _reg_path path = {
      .edge_pos = edge_pos, .next_node_pos = next_node_pos,
  };
...
static int _subset2node(struct reg_pattern *pattern, size_t begin_pos) {
  ...
  for (size_t pos = begin_pos; (v = state_node_pos(pattern, pos)); pos++) {
    ...
    if (eval_subset_len == list_len(v->subset)) {
      ...
      if (i == eval_subset_len)
        return pos; // the subset is exist
    }
  }
...
static void _sort_minsubset(struct reg_list *minsubset, size_t begin_idx,
                            size_t len) {
...
static struct reg_list *_new_minsubset(struct reg_pattern *pattern) {
  ...
  // foreach dfa node
  struct reg_node *v = NULL;
  for (size_t pos = pattern->dfa_start_state_pos;
       (v = state_node_pos(pattern, pos)); pos++) {
    struct min_node node = {
        .state_pos = v->node_pos, .subset = 0,
    };
...
static void _free_minsubset(struct reg_pattern *pattern,
                            struct reg_list *minsubset) {
  list_free(minsubset);
}

static inline int _split(struct reg_pattern *pattern,
                         struct reg_list *minsubset, size_t begin_idx,
                         size_t len) {
  ...
  for (size_t edge_pos = 1; edge_pos <= edge_len; edge_pos++) {
    ...
    for (size_t i = begin_idx; i < begin_idx + len; i++) {
      ...
      if ((!next_node) ||
          (next_node && len > 1 && cur_subset != next_node->subset_tag)) {
        if (per_split_subset < 0)
          per_split_subset = cur_splite_subset;
        if (per_split_subset != cur_splite_subset)
          continue;
...
static void _mark(struct reg_pattern *pattern, struct reg_list *minsubset) {
  ...
  for (i = 0; (v = list_idx(minsubset, i)); i = j) {
    ...
    size_t merge_pos = 0;
    for (j = i + 1; (nv = list_idx(minsubset, j)) && (v->subset == nv->subset);
         j++) {
      if (merge_pos == 0)
...
static void _merge(struct reg_pattern *pattern, struct reg_list *minsubset) {
  ...
  for (size_t pos = pattern->dfa_start_state_pos;
       (node = state_node_pos(pattern, pos)); pos++) {
    ...
    foreach_edge(v, node) {
      ...
      struct reg_node *next_node = state_node_pos(pattern, next_node_pos);
      next_node_pos =
          (next_node->merge_pos) ? (next_node->merge_pos) : (next_node_pos);
      if (merge_pos)
